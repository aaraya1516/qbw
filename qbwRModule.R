...
getMrnaData<-function() as.matrix(read.table('http://web.mit.edu/sgosline/Public/files/mrnaData.tab',sep='\t',as.is=T,header=T,row.names=1,fill=T)[,-missingpats])
getPatientData<-function(){
  tab=read.table("http://web.mit.edu/sgosline/Public/files/patientData.csv",sep=',',as.is=T,header=T,fill=T,row.names=1)[-missingpats,]
